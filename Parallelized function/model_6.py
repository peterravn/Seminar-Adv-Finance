...
def fit_model(hour):
    ...
    predictions = new_results.predict(start=len(y_train_deseason[hour]), end=len(y_train_deseason[hour]) + len(y_test_deseason[hour]) - 1, exog=pca_test[hour]).reshape(-1, 1)
...
if __name__ == '__main__':
    ...
    np.save('Parallelized function/model_6.npy', model_6)
    np.save('Parallelized function/combined_predictions_model_6.npy', combined_predictions_model_6)
